...
type AccomodationValidationSchema = z.infer<typeof accomodationSchema>;
...
const Page: NextPageWithLayout = () => {
  ...
  const { mutateAsync, isLoading } =
    api.accomodation.addAccomodation.useMutation({
      onSuccess: () => {
        toast({
          description: "You have succesfully added a new Accomodation",
        });
        reset();
      },
      onError: (error) => {
  ...
  const onSubmit: SubmitHandler<AccomodationValidationSchema> = async (
    data,
  ) => {
    try {
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<AccomodationValidationSchema>({
    resolver: zodResolver(accomodationSchema),
  });
  return (
    <main className="mt-[50px] pl-5">
      <h3 className="text-2xl font-medium ">New Accomodation</h3>
      <Toaster />
        ...
        <section className="relative flex flex-col space-y-[30px] pt-10 ">
          <ItemLayout>
            <AssetLabel
              ...
              caption="Enter a common name for this Accomodation"
            />
            <Input placeholder="Singita Lodges" {...register("name")} />
          </ItemLayout>
          ...
          <ItemLayout>
            <AssetLabel
              label="Pictures"
              caption="Add pictures for this accomodation to add visual appetite"
            />
            <div>
